    ...
    <div class="z-50 absolute bg-gray-800 rounded w-64 mt-4" x-show="isOpen">
        <ul>
            ...
            <li class="border-b text-sm border-gray-700">
                <a href="{{ route('movies.show', $result['id']) }}" class="hover:bg-gray-700 px-3 py-3 flex items-center" @if($loop->last)  @keydown.tab.exact="isOpen = false" @endif>
                    @if ($result['poster_path'])
                    <img src="https://image.tmdb.org/t/p/w92/{{ $result['poster_path'] }}" alt="{{ $result['title'] }}" class="w-8">
                    @else
                    <img src="https://via.placeholder.com/58x75" alt="{{ $result['title'] }}" class="w-8">
                    @endif
            ...
            @empty
            <li class="border-b text-sm border-gray-700">
                <a href="#" class="block hover:bg-gray-700 px-3 py-3">No results for "{{$search}}"</a>
            </li>
            @endforelse
